...
- [x] `元に戻す` が MSIX 版で完全復元すること（コミット `d5aa336` で堅牢化済み・OneDrive 配下含む）
- [x] 一度適用済みフォルダへの再適用でアイコンが更新されること（コミット `d5aa336` のハッシュ命名で解消）
...
- ICO ファイルは `_folderly\cover_<hash8>.ico`（アンダースコア始まり + ハッシュ付き）で保存。OneDrive がドット始まりディレクトリを dehydrate/削除するため `.folderly` から変更（コミット `70ee0d0`）。ハッシュはパス毎に変わるため Explorer の古いキャッシュが残らない。
...
- [x] 一度適用済みのフォルダに再適用 → アイコンが即時更新される（ハッシュ付き命名によりキャッシュが無効化される）— OneDrive 配下で確認済み（コミット `d5aa336`）
