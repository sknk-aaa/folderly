...
- Store candidate version: `1.6.9.0`
- Store upload candidate: `_out\Folderly_1.6.9.0_x64_store.msix`
...
| Current version | `1.6.9.0`（アプリ表示 `1.6.9`） |
| Min OS | Windows 10 1809 (`10.0.17763.0`) |

バージョンルール：Microsoft Store は4桁目が非ゼロの MSIX を拒否する。`1.6.9.0` は OK、`1.0.0.17` は NG。バージョンは Package.appxmanifest の `Version` と `Folderly.App.csproj` の `<Version>` の両方を更新する。次リリースも 4 桁目 0。
...
```powershell
$msix = '_out\Folderly_1.6.9.0_x64_store.msix'
$verify = '_out\verify_store_msix_manifest_1.6.9.0'
$makeappx = 'C:\Program Files (x86)\Windows Kits\10\bin\10.0.26100.0\x64\makeappx.exe'
...
- Store upload: `_out\Folderly_1.6.9.0_x64_store.msix`（署名なし）
- Local install: `_out\Folderly_1.6.9.0_x64_sideload.msix`（署名済み）

```powershell
$ErrorActionPreference = 'Stop'
$publisher = 'CN=F27FAE8B-A689-44D3-AB88-09E593D2DA9E'
$root = (Resolve-Path .).Path
$storeMsix = Join-Path $root '_out\Folderly_1.6.9.0_x64_store.msix'
$sideloadMsix = Join-Path $root '_out\Folderly_1.6.9.0_x64_sideload.msix'
...
```powershell
Get-FileHash .\_out\Folderly_1.6.9.0_x64_store.msix -Algorithm SHA256
```
