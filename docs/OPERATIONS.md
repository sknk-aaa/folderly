...
> Latest Partner Center draft: 2026-08-30. Confirmed from the current Partner Center screen after the Japanese listing update.
...
**Keywords**（検索用語・7枠）: `フォルダ アイコン 変更` / `フォルダ アイコン カスタマイズ` / `アイコン 変更 ツール` / `フォルダー デザイン 変更` / `デスクトップ フォルダ アイコン` / `フォルダ画像変更` / `デスクトップ フォルダ アイコン`
...
> Latest Partner Center draft: 2026-08-29. Confirmed from the current Partner Center screen after the English ASO test update.
...
Change plain Windows folder icons into visual shortcuts for your desktop and File Explorer.
...
- Organize desktop and File Explorer folders visually
- Add color tags and labels for different folder types
- Customize folders from File Explorer's right-click menu
- Adjust image position, scale, and display mode before applying
- Restore any folder from history whenever you want

Folderly works locally on your PC. No subscription, no ads, and your images stay on your device.

Note: Folderly may briefly reopen the target File Explorer window after applying an icon. This refreshes File Explorer so the new folder icon appears right away.

**Keywords**（Partner Center draft）: `folder icons` / `change folder icon` / `folder icon maker` / `windows folder icon` / `custom folder icon` / `desktop folder icons` / `photo folder icon`
