...
| Current version | `1.4.0.0`（アプリ表示 `1.4.0`） |
...
```powershell
$msix = '_out\Folderly_1.4.0.0_x64_store.msix'
$verify = '_out\verify_store_msix_manifest_1.4.0.0'
$makeappx = 'C:\Program Files (x86)\Windows Kits\10\bin\10.0.26100.0\x64\makeappx.exe'
...
- Store upload: `_out\Folderly_1.4.0.0_x64_store.msix`（署名なし）
- Local install: `_out\Folderly_1.4.0.0_x64_sideload.msix`（署名済み）

```powershell
$ErrorActionPreference = 'Stop'
$publisher = 'CN=F27FAE8B-A689-44D3-AB88-09E593D2DA9E'
$root = (Resolve-Path .).Path
$storeMsix = Join-Path $root '_out\Folderly_1.4.0.0_x64_store.msix'
$sideloadMsix = Join-Path $root '_out\Folderly_1.4.0.0_x64_sideload.msix'
...
```powershell
Get-FileHash .\_out\Folderly_1.4.0.0_x64_store.msix -Algorithm SHA256
```
