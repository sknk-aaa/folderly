...
- ブログ記事 4本公開済み（JP 2本・EN 2本）。リポ `C:\src\folderly-web`。
  - JP: `/blog/windows-folder-icon-change/`, `/blog/folder-icon-not-changing/`
  - EN: `/en/blog/windows-folder-icon-change/`, `/en/blog/folder-icon-not-changing/`
- 全 CTA は LP（folderlyapp.com）一本。hreflang/sitemap 整備済み。
- 戦略の正本は `docs/MARKETING.md`。要点：日本＝SEO記事、海外＝Store ASO＋Product Hunt。
- 次の一手：日本語記事3本目（テーマ未定）、note投稿、Store ASO最適化、Product Hunt。
